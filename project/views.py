...
def ticket_status_default(request, slug):
    ...
    TicketStatus.objects.bulk_create(
        [TicketStatus(name='New', slug=slugify('New'), color='#999999', project=project, order=1),
         TicketStatus(name='In progress', slug=slugify('In progress'), color='#729fcf', project=project, order=2),
         TicketStatus(name='Ready for test', slug=slugify('Ready for test'), color='#4e9a06', project=project, order=3),
...
def ticket_status_order(request, slug, ticket_slug):
    ticket = TicketStatus.objects.get(id=ticket_slug,
                                      project=Project.objects.get(slug=slug, organization=request.user.organization))
